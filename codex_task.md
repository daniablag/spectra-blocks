...
Codex должен научиться формировать корректный JSON/HTML-код для Gutenberg-блоков Spectra по тексовому описанию пользователя, который:

- соответствует структуре, принятой в Spectra
- использует только допустимые атрибуты из плагина (а не придумывает свои)
- генерирует рабочий код, который можно вставить в редактор WordPress
- опирается на знания из исходного кода плагина и примеров
...
- `/examples/*.notes.md` — наблюдения по каждому отдельному блоку.
  ...
  - Каждый файл содержит:
    ...
    - какие поля попадают в JSON
    - примеры сформированных блоков с описанием
